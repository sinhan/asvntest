...
public abstract class AbstractProperty implements JMeterProperty
{
   ...
   public Object clone()
   {
      try
      {
         AbstractProperty prop = (AbstractProperty) this.getClass()
               .newInstance();
         prop.name = name;
   ...
   protected JMeterProperty getBlankProperty()
   {
      try
      {
         ...
         if (prop instanceof NullProperty) { return new StringProperty(); }
         return prop;
      }
      catch (Exception e)
      {
   ...
   {
      if (item == null) { return new NullProperty(); }
   ...
   {
      if (item instanceof JMeterProperty) { return (JMeterProperty) item; }
      if (item instanceof TestElement) { return new TestElementProperty(
            ((TestElement) item).getPropertyAsString(TestElement.NAME),
            (TestElement) item); }
      if (item instanceof Collection) { return new CollectionProperty(""
            + item.hashCode(), (Collection) item); }
      if (item instanceof Map) { return new MapProperty("" + item.hashCode(),
            (Map) item); }
      return null;
   }
